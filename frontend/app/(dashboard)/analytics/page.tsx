        ...
        <div className="flex gap-1 bg-slate-100 rounded-xl p-1">
          {(['overview', 'detailed'] as Tab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all capitalize ${tab === t ? 'bg-white shadow text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}
            >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-slate-800">Ticket Trend ({days} days)</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <h3 className="font-semibold text-slate-800 mb-4">Team Workload</h3>
                ...
                <div className="space-y-2">
                  {workload.slice(0, 8).map((row: any) => (
                    <div key={row.user?.id} className="flex items-center gap-3">
                      <span className="text-sm text-slate-700 w-32 truncate">{row.user?.name ?? row.name}</span>
                      ...
                      <span className="text-xs text-slate-500 w-8 text-right">{(row.open ?? 0) + (row.inProgress ?? 0)}</span>
                    </div>
                  ))}
                </div>
              ) : (
        ...
        <div className="space-y-6">
            ...
            <div className="flex gap-2">
              {(['7d', '30d', '90d'] as Range[]).map((r) => (
                <button
                  key={r}
                  onClick={() => setRange(r)}
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition-all ${range === r ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                >
                  {r}
                </button>
              ))}
          ...
          <div className="bg-white rounded-xl border border-slate-200">
            <div className="px-5 py-4 border-b border-slate-100">
              <h3 className="font-semibold text-slate-800">Open Tickets — Age</h3>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    ...
                    <th className="text-right px-5 py-3 text-xs font-semibold text-slate-500 uppercase">Age</th>
                  </tr>
                </thead>
                  ...
                  {tickets.filter((t) => !['DONE', 'CLOSED'].includes(t.status)).slice(0, 20).map((t) => {
                    ...
                    return (
                      <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-5 py-3 font-mono text-xs text-slate-500">{t.ticketId}</td>
                        <td className="px-5 py-3 text-slate-800 max-w-xs truncate">{t.title}</td>
                        <td className="px-5 py-3">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_BADGE[t.status] ?? ''}`}>
                            {t.status.replace('_', ' ')}
                          </span>
                        </td>
                        <td className="px-5 py-3 text-xs text-slate-600">{t.priority}</td>
                        <td className="px-5 py-3 text-xs text-slate-600">{t.assignedTo?.name ?? '—'}</td>
                        <td className="px-5 py-3 text-right"><AgeBadge days={ageDays} /></td>
                      </tr>
                    );
                  })}
                  {tickets.filter((t) => !['DONE', 'CLOSED'].includes(t.status)).length === 0 && (
                    <tr><td colSpan={6} className="px-5 py-8 text-center text-slate-400">No open tickets</td></tr>
                  )}
                </tbody>
              </table>
            </div>
